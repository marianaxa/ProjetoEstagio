...
//echo $idlote;
$loteRecebido->select("SELECT idlote_sementes, nome_vulgar, nome_cientifico, familia, data_chegada, nome_fornecedor, renasem, telefone, rua, bairro, cidade, estado, cep FROM lote, especie, fornecedor, endereco WHERE idlote_sementes='$idlote' and especieFK=id_especie AND origemFK=id_fornecedor AND enderecoFK = id_endereco");

foreach ($loteRecebido->result() as $loteRecebido ){
	$idlote= $loteRecebido['idlote_sementes'];
	$especie = $loteRecebido['nome_vulgar'];
	$nomecientifico = $loteRecebido['nome_cientifico'];
	$familia = $loteRecebido['familia'];
